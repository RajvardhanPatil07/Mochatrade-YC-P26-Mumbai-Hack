...
def _fresh_research_observation(symbol: str) -> dict | None:
    """Return a display-only live observation without promoting it to Market Truth."""
    try:
        live = get_live_snapshot(force=True)
    except Exception:
        return None
    return next(
        (
            item
            for item in live.get("observations", [])
            if item.get("symbol") == symbol
            and item.get("observed_price") is not None
            and item.get("source", {}).get("status") == "FRESH"
        ),
        None,
    )
...
def install_judge_routes(app: FastAPI, pipeline, risk_gateway) -> None:
    ...
    def live_baseline(payload: LiveBaselineRequest):
        ...
        qualified = reference is not None
        research = None if qualified else _fresh_research_observation(symbol)
        if research is not None:
            reference = research["observed_price"]
        ...
        source = research.get("source", {}) if research else {}
        reference_status = (
            market.get("reference_status")
            if qualified
            else "UNQUALIFIED_RESEARCH_ONLY"
        )
        return {
            "available": True,
            "qualified": qualified,
            "data_mode": (
                "LIVE_BASELINE_WITH_SYNTHETIC_VENUE_ATTACK"
                if qualified and payload.inject_attack
                else "LIVE_BASELINE"
                if qualified
                else "LIVE_RESEARCH_BASELINE_WITH_SYNTHETIC_VENUE_ATTACK"
                if payload.inject_attack
                else "LIVE_RESEARCH_BASELINE"
            ),
            "symbol": symbol,
            "clock": _clock(now),
            "reason": (
                None
                if qualified
                else "A fresh live observation is available, but it is display-only and does not satisfy the independent-provider quorum. New risk remains fail-closed."
            ),
            "provenance": {
                "underlying_evidence": (
                    "LIVE_BACKEND_SNAPSHOT"
                    if qualified
                    else "LIVE_RESEARCH_OBSERVATION"
                ),
                "source": source.get("name") if research else None,
                "venue_mark": "SYNTHETIC_INJECTED" if payload.inject_attack else "REFERENCE_ALIGNED_CONTROL",
                "policy": "PRODUCTION_GATE",
                "trade_submitted": False,
            },
            "market_truth": {
                "reference_price": market.get("reference_price") if qualified else float(reference_decimal),
                "reference_status": reference_status,
                "provider_count": market.get("provider_count") if qualified else 1,
                "venue_count": market.get("venue_count") if qualified else 1,
                "confidence": market.get("confidence") if qualified else 0,
            },
            "venue_mark": float(mark),
            "attack_bps": float(payload.attack_bps) if payload.inject_attack else 0.0,
            "divergence_bps": (
                market.get("divergence_bps")
                if qualified
                else float(payload.attack_bps) if payload.inject_attack else 0.0
            ),
            "decision": result,
