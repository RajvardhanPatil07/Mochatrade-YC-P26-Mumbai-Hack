...
def _resolve_reference(payload: WarRoomRequest, snapshot: dict) -> tuple[Decimal, str, bool]:
    ...
    if payload.mode == "LIVE" and payload.baseline_price is not None:
        return payload.baseline_price, "CLIENT_LIVE_DISPLAY_SNAPSHOT", False
...
def _war_room_risk_request(
    ...
    data_mode = (
        "SYNTHETIC_DEMO"
        if synthetic
        else "LIVE_DERIVED_DEMO"
        if baseline_source == "LIVE_QUALIFIED_REFERENCE"
        else "LIVE_UNQUALIFIED_DEMO"
    )
    return request, {
...
def _story(payload: WarRoomRequest, result: dict, provenance: dict) -> dict:
    ...
    if provenance["data_mode"] == "LIVE_UNQUALIFIED_DEMO":
        scenario_copy = {
            "headline": "Live display price received; qualified Market Truth is unavailable.",
            "detail": "The observed baseline remains visible, while the production gate blocks new risk until an independent evidence quorum exists.",
        }
    return {
        **scenario_copy,
        **provenance,
