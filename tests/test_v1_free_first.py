...
def test_live_mode_uses_display_baseline_but_keeps_unqualified_risk_fail_closed():
    payload = WarRoomRequest(
        scenario="NORMAL",
        mode="LIVE",
        symbol="NVDA",
        baseline_price=220.25,
    )
    ...
    assert provenance["data_mode"] == "LIVE_UNQUALIFIED_DEMO"
    assert provenance["baseline_source"] == "CLIENT_LIVE_DISPLAY_SNAPSHOT"
