...
export function marketConnection(snapshot: ShadowSnapshot | null, displaySnapshots: DisplaySnapshot[]): ConnectionState {
  if (displaySnapshots.some((item) => item.status === "AVAILABLE" && !item.is_delayed)) return "live";
  const providerIsLive = snapshot?.providers.some((provider) =>
    ["alpaca", "hyperliquid", "twelve-data"].includes(provider.id)
    && ["AVAILABLE", "LIMITED", "AUTHENTICATED"].includes(provider.status)
  );
  if (providerIsLive) return "live";
  if (displaySnapshots.some((item) => item.status === "AVAILABLE") || snapshot?.decisions.length || snapshot?.yahoo?.observations?.length) return "delayed";
  return snapshot ? "offline" : "connecting";
}

export function latestMarketUpdate(snapshot: ShadowSnapshot | null, displaySnapshots: DisplaySnapshot[]): string | null {
  const timestamps = [
    ...displaySnapshots.map((item) => item.event_time),
    ...(snapshot?.decisions.map((decision) => decision.timestamp) ?? []),
    snapshot?.yahoo?.fetched_at ?? null,
