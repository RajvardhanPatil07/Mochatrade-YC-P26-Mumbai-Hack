...
import { assetsFromSnapshot, marketConnection, latestMarketUpdate, mergeDisplaySnapshots } from "./market";
...
describe("batch display snapshots", () => {
  ...
  it("uses a fresh real-time display quote for the global market status", () => {
    const snapshot = {
      decisions: [{ timestamp: "2026-09-11T12:39:07Z" }],
      providers: [{ id: "alpaca", status: "CONNECTING" }],
      yahoo: null,
    } as unknown as ShadowSnapshot;
    const display: DisplaySnapshot = {
      symbol: "AAPL", name: "Apple Inc.", exchange: "NASDAQ", currency: "USD",
      status: "AVAILABLE", price: 226.91, previous_close: 224.13, change_pct: 1.24,
      event_time: "2026-09-11T14:23:40Z", bid: 226.9, ask: 226.92,
      day_volume: 12_500_000, feed: "iex", is_delayed: false,
    };

    expect(marketConnection(snapshot, [display])).toBe("live");
    expect(latestMarketUpdate(snapshot, [display])).toBe("2026-09-11T14:23:40Z");
  });
});
