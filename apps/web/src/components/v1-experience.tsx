...
import { incidentImpactReducer, replayPassportId } from "@/lib/war-room-impact";
...
type WarRoomResult = {
  ...
  result: {
    ...
    passport: { chain_hash?: string; content_hash?: string; claims?: { identity?: { timestamp?: string }; policy?: { policy_version?: string } } };
  };
};
...
export function WarRoom() {
  ...
  const [lastIncidentImpact, updateIncidentImpact] = useReducer(incidentImpactReducer, null);
  ...
  const apply = (next: WarRoomResult, label?: string) => {
    setData(next);
    setReplay(null);
    updateIncidentImpact({ type: "DECISION", decision: next });
  ...
  const compare = async () => {
    const passportId = replayPassportId(lastIncidentImpact, data?.result.passport_id ?? null);
    ...
    try {
      const response = await fetch(`${API_BASE}/v1/demo/war-room/replay`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ passport_id: passportId, mode: "WITHOUT_SAFETY_GATE" }),
      });
  ...
  return <div className="v1-page war-room">
    ...
    <section className="war-controls">
      ...
      <div className="baseline-readout"><span>Display baseline</span><strong>{usd(selectedAsset?.price)}</strong><small>LIVE can display this observation, but new risk stays blocked unless the provider quorum qualifies it.</small></div>
    </section>
    ...
    {!data ? <section className="war-empty">
      ...
      <section className="truth-stage">
        ...
        <div className="truth-core"><span>{liveUnqualified ? "OBSERVED BASELINE" : "MARKET TRUTH"}</span><strong>{usd(liveUnqualified ? data.story.baseline_price : data.result.market.reference_price)}</strong><div className="confidence-ring">{Math.round((data.result.market.confidence ?? 0) * 100)}%<small>confidence</small></div><small>{liveUnqualified ? "UNQUALIFIED · FAIL CLOSED" : data.result.market.reference_status?.replaceAll("_", " ")}</small></div>
      ...
      {lastIncidentImpact && <section className="incident-impact" data-passport-id={lastIncidentImpact.passportId}>
        <header className="incident-impact-head">
          <div><h2>INCIDENT IMPACT</h2><p>Requested exposure restricted</p></div>
          <div className="incident-states"><span>CURRENT STATE <b>{data.result.market.asset_state.replaceAll("_", " ")} · {data.result.action.replaceAll("_", " ")}</b></span><span>LAST MATERIAL INCIDENT <b>{lastIncidentImpact.scenario.replaceAll("_", " ")} · {lastIncidentImpact.divergenceBps == null ? "—" : `${lastIncidentImpact.divergenceBps.toFixed(0)} bps`} · {lastIncidentImpact.action.replaceAll("_", " ")}</b></span></div>
        </header>
        <div className="incident-flow" aria-label="Incident consequence flow">
          <div><span>VENUE ERROR</span><strong>{lastIncidentImpact.divergenceBps == null ? "—" : `${lastIncidentImpact.divergenceBps >= 0 ? "+" : ""}${lastIncidentImpact.divergenceBps.toFixed(0)} bps`}</strong></div><i>↓</i>
          <div><span>REQUESTED ORDER</span><strong>{lastIncidentImpact.requestedLeverage}× · {usd(lastIncidentImpact.requestedNotionalUsd)}</strong></div><i>↓</i>
          <div><span>MARKETBRIDGE</span><strong>{lastIncidentImpact.action.replaceAll("_", " ")}</strong></div><i>↓</i>
          <div className="impact-outcome"><strong>{usd(lastIncidentImpact.additionalExposurePreventedUsd)}</strong><span>ADDITIONAL EXPOSURE PREVENTED</span></div>
        </div>
        <div className="incident-comparison">
          <article><span>WITHOUT MARKETBRIDGE</span><strong>ALLOW</strong><small>Ungated request proceeds</small><dl><div><dt>Requested leverage</dt><dd>{lastIncidentImpact.requestedLeverage}×</dd></div><div><dt>Requested exposure</dt><dd>{usd(lastIncidentImpact.requestedNotionalUsd)}</dd></div></dl></article>
          <b>VS</b>
          <article><span>WITH MARKETBRIDGE</span><strong>{lastIncidentImpact.action.replaceAll("_", " ")}</strong><small>RiskGateway decision</small><dl><div><dt>Permitted leverage</dt><dd>{lastIncidentImpact.permittedLeverage}×</dd></div><div><dt>Permitted exposure</dt><dd>{usd(lastIncidentImpact.permittedNotionalUsd)}</dd></div></dl></article>
        </div>
        <dl className="impact-metrics">
          <div><dt>ADDITIONAL EXPOSURE PREVENTED</dt><dd>{usd(lastIncidentImpact.additionalExposurePreventedUsd)}</dd></div>
          <div><dt>LEVERAGE REDUCTION</dt><dd>{lastIncidentImpact.requestedLeverage}× → {lastIncidentImpact.permittedLeverage}×</dd></div>
          <div><dt>SIMULATED MARK-TO-TRUTH EXPOSURE</dt><dd>{usd(lastIncidentImpact.simulatedPriceGapExposureUsd)}</dd></div>
          <div><dt>PRICE GAP VS ILLUSTRATIVE INITIAL MARGIN</dt><dd>{lastIncidentImpact.priceGapAsPctOfInitialMargin == null ? "—" : `${lastIncidentImpact.priceGapAsPctOfInitialMargin.toFixed(1)}%`}</dd></div>
        </dl>
        <p className="incident-disclaimer">Counterfactual risk illustration. No claim is made that an ungated order would have executed or that a realized loss was avoided.</p>
        {replay && <p className="incident-replay"><strong>COUNTERFACTUAL REPLAY VERIFIED</strong><span>{replay.replayed_action.replaceAll("_", " ")} without gate · {replay.original_action.replaceAll("_", " ")} with MarketBridge</span></p>}
      </section>}
