...
import { assetsFromSnapshot, latestMarketUpdate, marketConnection, mergeDisplaySnapshots, type ConnectionState, type MarketAsset } from "@/lib/market";
...
export function MarketDataProvider({ children }: { children: React.ReactNode }) {
  ...
  const loadSnapshot = useCallback(async () => {
    try {
      ...
      setError(friendlyError(status)); setLoading(false);
      return null;
    }
  }, [accept]);
  ...
  useEffect(() => {
    ...
    try {
      socket = new WebSocket(`${wsOrigin}/v1/shadow/ws`);
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => void loadDisplaySnapshots(), 15_000);
    return () => window.clearInterval(timer);
  }, [alpacaConfigured, loadDisplaySnapshots]);

  const connection = error ? "offline" : marketConnection(snapshot, displaySnapshots);
  const value = useMemo<MarketDataContextValue>(() => ({ assets: mergeDisplaySnapshots(assetsFromSnapshot(snapshot), displaySnapshots), snapshot, connection, error, loading, lastUpdated: latestMarketUpdate(snapshot, displaySnapshots), refresh }), [snapshot, displaySnapshots, connection, error, loading, refresh]);
