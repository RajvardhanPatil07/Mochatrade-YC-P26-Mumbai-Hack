          ...
          {live && (
            <div className="judge-result">
              ...
              {!live.available ? (
                ...
                <>
                  {!live.qualified && <div className="judge-unavailable"><strong>LIVE PRICE · NOT QUALIFIED MARKET TRUTH</strong><p>{live.reason}</p><small>{live.provenance.source ?? "Display-only research feed"}</small></div>}
                  <div className="judge-metrics">
                    <div><span>{live.qualified ? "Market Truth" : "Observed baseline"}</span><strong>{usd(live.market_truth?.reference_price)}</strong><small>{live.market_truth?.reference_status}</small></div>
                    <div><span>Venue mark</span><strong>{usd(live.venue_mark)}</strong><small>{live.divergence_bps?.toFixed(1)} bps divergence</small></div>
                    <div><span>{live.qualified ? "Independent families" : "Observed families"}</span><strong>{live.market_truth?.provider_count ?? 0}</strong><small>{live.market_truth?.venue_count ?? 0} source families</small></div>
